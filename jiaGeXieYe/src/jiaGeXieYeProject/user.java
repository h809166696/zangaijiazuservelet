	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (map.containsKey("action")) {
			...
			if (actionStr.equals("register")) {
				//0 账号重复 1 成功 -1 昵称重复 -2插入失败
				String account = map.get("phonenum")[0];
...
//				        "gb2312");
			if (manager.checkUserIsExist("user", account, "phonenum")) {
				out.write("0");
			}else if(manager.checkUserIsExist("usertable", name, "uname"))
			{out.write("-1");}else{
				String[] keyArray = {"uname","password","phonenum"};
				String[] valueArray = {name,password,account};
				boolean isSuccess = manager.addData("user", keyArray, valueArray);
				if (isSuccess) {
					out.write("1");
				}else
				{
				...
				String password = map.get("password")[0];
				if (manager.checkUserIsExist("user", account, "phonenum")) {
					String[] strArray = {"password","uname","uid","phonenum"};
					List<Map<String, Object>> data = manager.getOtherText("user", "phonenum", account,strArray);
				...
				if (checkStrArray(valueArray)) {
					 if (manager.addData("shoosedb", keyArray, valueArray)) {
						 out.write(gid);
						}else
						{
							out.write("-1");
